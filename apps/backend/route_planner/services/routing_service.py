...
class RoutingService:
    ...
    def geocode_location(self, location_name: str) -> Tuple[float, float]:
        """
        Geocode a location name to coordinates, or reverse geocode if coordinates are provided.
        """
        ...
        # Detect if input is coordinates (e.g., "-1.2405, 36.7276")
        coord_match = re.match(
            r"^\s*(-?\d+(\.\d+)?),\s*(-?\d+(\.\d+)?)\s*$", location_name
        )
        if coord_match:
            # Use reverse geocoding
            lat, lng = float(coord_match.group(1)), float(coord_match.group(3))
            try:
                response = self._make_request(
                    f"geocode/reverse?point.lat={lat}&point.lon={lng}"
                )
                if not response.get("features"):
                    raise GeocodingError(f"Coordinates not found: {location_name}")
                coordinates = response["features"][0]["geometry"]["coordinates"]
                cache.set(cache_key, coordinates, timeout=CACHE_TIMEOUT)
                return coordinates
            except Exception as e:
                raise GeocodingError(
                    f"Reverse geocoding failed for {location_name}: {str(e)}"
                )
        else:
            # Use normal geocoding
            try:
                response = self._make_request(f"geocode/search?text={location_name}")
                if not response.get("features"):
    ...
    def _calculate_route_segment(
        ...
        try:
            response = self._make_request(
                "v2/directions/driving-hgv", method="POST", data=data
            )
            ...
            raise RouteCalculationError(f"Route segment calculation failed: {str(e)}")
