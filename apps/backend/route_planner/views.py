...
class TripViewSet(viewsets.ModelViewSet):
    ...
    def plan_route(self, request, pk=None):
        ...
        try:
            ...
            plan_data = {
                ...
                "stops": RestStopSerializer(stops, many=True).data,
    ...
    def _get_route(self, current_coords, pickup_coords, dropoff_coords, api_key):
        ...
        combined_route = {
            ...
            + second_leg["duration"]
            + 3600,  # Add 1 hour (3600 seconds) for pickup
    ...
    def _get_route_segment(self, start_coords, end_coords, api_key):
        ...
        body = {"coordinates": [start_coords, end_coords], "radiuses": [-1, -1]}
    ...
    def _calculate_fuel_stops(self, total_distance_meters):
        ...
        for i in range(1, num_fuel_stops + 1):
            distance_at_stop = (
                (i * 1000 * 1609.34) if i < num_fuel_stops else total_distance_meters
            )
            fuel_stops.append(
                {
    ...
    def _generate_eld_logs(self, trip, route_data):
        ...
        while total_elapsed_drive_time < total_drive_time:
            ...
            log_sheet["activities"].append(
                {
                    ...
                    "end_time": (simulation_time + timedelta(minutes=15)).strftime(
                        "%H:%M"
                    ),
                    "duration": 0.25,
                }
            ...
            if (
                total_elapsed_drive_time >= route_data["legs"][0]["duration"] / 3600
                and len(log_sheet["activities"]) < 4
            ):
                # Add pickup (1 hour)
                log_sheet["activities"].append(
                    {
                        "status": "ON",
                        "start_time": simulation_time.strftime("%H:%M"),
                        "end_time": (simulation_time + timedelta(hours=1)).strftime(
                            "%H:%M"
                        ),
                        "duration": 1,
            ...
            if simulation_time.hour < 24:
                off_duty_hours = (
                    24 - simulation_time.hour - (1 if simulation_time.minute > 0 else 0)
                )
